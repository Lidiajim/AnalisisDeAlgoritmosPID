...
class HOGDetect:
    ...
    def __init__(self, cell_size=(8, 8), block_size=(2, 2), block_stride=(8, 8), nbins=9,
        ...
        self.scale = scale
        self.bin_width = np.deg2rad(bin_width)
        ...
        for i in range(n_cells_y):
            for j in range(n_cells_x):
                ...
                for bin_idx in range(self.nbins):
                    ...
                    x1 = int(center_x - length * np.cos(angle))
                    y1 = int(center_y - length * np.sin(angle))
